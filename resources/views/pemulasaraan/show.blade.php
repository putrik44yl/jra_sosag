...
<div class="container">
    ...
    <div class="mt-3">
        <a href="{{ route('pemulasaraan.index') }}" class="btn btn-secondary">Kembali</a>
        <a href="{{ route('pemulasaraan.edit', $pemulasaraan->id) }}" class="btn btn-warning">Edit</a>
        <form action="{{ route('pemulasaraan.destroy', $pemulasaraan->id) }}" method="POST" class="d-inline">
            @csrf
            @method('DELETE')
